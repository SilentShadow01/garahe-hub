<?php
   ...
   if (mysqli_num_rows($results_pros) > 0)
    ...
    $sql_pros = "SELECT count(*) AS `Count` FROM usermanagement_tbl WHERE accountStatus = 'pending'";
    $results_pros = mysqli_query($conn, $sql_pros);
    if (mysqli_num_rows($results_pros) > 0)
    {
        while($row_cat = mysqli_fetch_array($results_pros))
        {
            $pendingUser = $row_cat['Count'];
        }
    }
    else 
    {
    ...
    $sql_pros = "SELECT count(*) AS `Count` FROM usermanagement_tbl WHERE accountStatus = 'verified'";
    $results_pros = mysqli_query($conn, $sql_pros);
    if (mysqli_num_rows($results_pros) > 0)
    {
        while($row_cat = mysqli_fetch_array($results_pros))
        {
            $verifiedUsers = $row_cat['Count'];
        }
    }
    else 
    {
    ...
    $sql_pros = "SELECT count(*) AS value_sum FROM users_reservation_tbl WHERE `reservationStatus` = 'Pending' AND `reservationType` = 'event'";
    $results_pros = mysqli_query($conn, $sql_pros);
    if (mysqli_num_rows($results_pros) > 0)
    {
        while($row_cat = mysqli_fetch_array($results_pros))
        {
            $pendingEvent = $row_cat['value_sum'];
        }
    } 
    else
    {
    ...
    $sql_pros = "SELECT count(*) AS value_sum FROM users_reservation_tbl WHERE `reservationStatus` = 'Pending' AND `reservationType` = 'dineIn'";
    $results_pros = mysqli_query($conn, $sql_pros);
    if (mysqli_num_rows($results_pros) > 0)
    {
        while($row_cat = mysqli_fetch_array($results_pros))
        {
            $pendingDineIn = $row_cat['value_sum'];
        }
    } 
    else
    {
    ...
    $sql_pros = "SELECT count(*) AS value_sum FROM user_delivery_tbl WHERE `modeOfClaim` = 'delivery' AND `orderStatus` = 'pending'";
    $results_pros = mysqli_query($conn, $sql_pros);
    if (mysqli_num_rows($results_pros) > 0)
    {
        while($row_cat = mysqli_fetch_array($results_pros))
        {
            $pendingDelivery = $row_cat['value_sum'];
        }
    }
    else
    {
