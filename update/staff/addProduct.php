        ...
        if ($_FILES['productImage']['size'] <= $maxFileSize) {
            ...
            if (move_uploaded_file($productImageTemp, $targetFile)) {
                ...
                if(mysqli_num_rows($checkResult) > 0) {
                    ...
                    $query = "INSERT INTO product_tbl (productName, productPrice, productID, productImage, productCategory, productType)
                            VALUES ('$productName', '$productPrice', '$productID', '$newFilename', '$productCategory', '$productStatus')";
...
if(isset($_POST['editRequestBtn']))
{
    ...
    $query = "UPDATE `product_tbl` SET `productName` = ?, `productPrice` = ?, `productType` = ?, `productCategory` = ? WHERE `productID` = ?";
    
    // Prepare the statement
    $stmt = $conn->prepare($query);

    // Bind the parameters and their types
    $stmt->bind_param("sssss", $editProductName, $editProductPrice, $editProductStatus, $productCategory, $editProductID);
