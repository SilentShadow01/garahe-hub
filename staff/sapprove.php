<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editOrderStatus'])) {
        ...
        $additionalSql = "SELECT * FROM user_delivery_tbl WHERE orderDate = '$orderDate' AND orderTime= '$orderTime' AND orderStatus = 'approve'";
        ...
        while ($additionalRow = mysqli_fetch_assoc($additionalResult))
        {
            ...
            // var_dump($rowID); 
            $updateQuery = "UPDATE user_delivery_tbl SET orderStatus = '$newOrderStatus' WHERE ID = '$rowID' AND orderStatus = 'approve'";
            $updateResult = mysqli_query($conn, $updateQuery);
        }
        
        $transactionNumber = $_POST['transactionNumber'];
        $admin_name = $_SESSION['admin_name'];
        $log_message = "<strong>$admin_name</strong> set the Order Status from <strong>To ship</strong> into <strong>$newOrderStatus</strong> where Transaction number is <strong>$transactionNumber</strong>.";
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4" style="background-color: #F2E8C6;">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom border-dark">
                    <h1 class="h2" id="currentPage"><i class="fa-solid fa-truck"></i>&nbsp Deliver > Approved</h1>
                </div>
                    ...
                    <table class="border border-secondary" id="tableDineIn">
                        ...
                        <tbody class="border border-secondary">
                            <?php
                                ...
                                $sql = "SELECT *
                                FROM user_delivery_tbl WHERE orderStatus = 'approve' AND modeOfClaim = 'delivery'
                                                            ...
                                                            <div class="container">
                                                                ...
                                                                <form method="POST">
                                                                    ...
                                                                    <div class="input-group mt-1">
                                                                        ...
                                                                        <option value="approve" class="bg-secondary text-light" <?= $row['orderStatus'] == 'approve' ? 'selected' : '' ?> disabled>Approved</option>
                                                                            <option value="to-deliver" class="bg-warning text-danger" <?= $row['orderStatus'] == 'to-deliver' ? 'selected' : '' ?>>To be Deliver</option>
                                                                        </select>
                                                                    </div>
