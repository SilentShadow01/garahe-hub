<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editOrderStatus'])) {
        ...
        $additionalSql = "SELECT * FROM user_delivery_tbl WHERE orderDate = '$orderDate' AND orderTime= '$orderTime' AND orderStatus = 'to-received'";
        ...
        while ($additionalRow = mysqli_fetch_assoc($additionalResult))
        {
            ...
            // var_dump($rowID); 
            $updateQuery = "UPDATE user_delivery_tbl SET orderStatus = '$newOrderStatus' WHERE ID = '$rowID' AND orderStatus = 'to-received'";
            $updateResult = mysqli_query($conn, $updateQuery);
        }
        
        $transactionNumber = $_POST['transactionNumber'];
        $admin_name = $_SESSION['admin_name'];
        $log_message = "<strong>$admin_name</strong> set the Order Status from <strong>To received</strong> into <strong>$newOrderStatus</strong> where Transaction number is <strong>$transactionNumber</strong>.";
        ...
        header("Location: ptoReceived.php");
    }
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4" style="background-color: #F2E8C6;">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom border-dark">
                    <h1 class="h2" id="currentPage"><i class="fa-solid fa-truck"></i>&nbsp Pick up > To Receive</h1>
                </div>
                    ...
                    <table class="border border-secondary" id="tableDineIn">
                        ...
                        <tbody class="border border-secondary">
                            <?php
                                ...
                                $sql = "SELECT *
                                FROM user_delivery_tbl WHERE orderStatus = 'to-received' AND modeOfClaim = 'pickUp'
                                                        ...
                                                        <div class="modal-body bg-body-tertiary">
                                                            <div class="container">
                                                                ...
                                                                <form method="POST">
                                                                    ...
                                                                    <div class="input-group mt-1">
                                                                        <strong class="font-num input-group-text col-4">Quantity</strong> 
                                                                        <input type="text" class="font-num form-control text-center" value="<?= $row['productQuantity']; ?>" readonly>   
                                                                    </div>
                                                                    <div class="input-group mt-1">
                                                                    ...
                                                                    <div class="input-group mt-1">
                                                                        ...
                                                                        <img src="../user/images/proof/pickup/<?= $row['proofOfPayment']; ?>" class="col-md-8" alt="Proof of Payment not found">
                                                                    </div>
                                                                    <div class="input-group mt-1">
                                                                        ...
                                                                        <select class="font-num form-control text-center" name="orderStatus" id="orderStatus" type="text" value="<?= $row['orderStatus']; ?>">
                                                                            <option value="to-received" class="bg-warning text-danger" <?= $row['orderStatus'] == 'to-received' ? 'selected' : '' ?> disabled>To Received</option>
                                                                            <option value="complete" class="bg-success text-light" <?= $row['orderStatus'] == 'complete' ? 'selected' : '' ?>>Complete</option> 
                                                                            <option value="cancelled" class="bg-danger text-light" <?= $row['orderStatus'] == 'cancelled' ? 'selected' : '' ?>>Cancel</option>
                                                                        </select>
                                                                    </div>
