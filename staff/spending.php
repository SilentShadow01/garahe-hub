<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editOrderStatus'])) {
        ...
        $additionalSql = "SELECT * FROM user_delivery_tbl WHERE orderDate = '$orderDate' AND orderTime= '$orderTime' AND orderStatus = 'pending'";
        ...
        while ($additionalRow = mysqli_fetch_assoc($additionalResult))
        {
            ...
            // var_dump($rowID); 
            $updateQuery = "UPDATE user_delivery_tbl SET orderStatus = '$newOrderStatus' WHERE ID = '$rowID' AND orderStatus = 'pending'";
            $updateResult = mysqli_query($conn, $updateQuery);
        }
        $transactionNumber = $_POST['transactionNumber'];
        $admin_name = $_SESSION['admin_name'];
        $log_message = "<strong>$admin_name</strong> set the Order Status from <strong>Pending</strong> into <strong>$newOrderStatus</strong> where Transaction number is <strong>$transactionNumber</strong>.";
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4" style="background-color: #F2E8C6;">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom border-dark">
                    <h1 class="h2" id="currentPage"><i class="fa-solid fa-file-pen"></i>&nbsp Deliver > Pending</h1>
                </div>
                    ...
                    <table class="border border-secondary" id="tableDineIn">
                        ...
                        <tbody class="border border-secondary">
                            <?php
                                ...
                                $sql = "SELECT *
                                FROM user_delivery_tbl WHERE orderStatus = 'pending' AND modeOfClaim = 'delivery'
                                                            ...
                                                            <div class="container">
                                                                ...
                                                                <form method="POST">
                                                                    ...
                                                                    <div class="input-group mt-1">
                                                                        ...
                                                                        <img src="../user/images/proof/pickup/<?= $row['proofOfPayment']; ?>" class="col-md-8">
                                                                    </div>
                                                                    <div class="input-group mt-1">
                                                                        ...
                                                                        <option value="pending" class="bg-secondary text-light" <?= $row['orderStatus'] == 'pending' ? 'selected' : '' ?> disabled>Pending</option>
                                                                            <option value="approve" class="bg-info text-dark" <?= $row['orderStatus'] == 'approve' ? 'selected' : '' ?>>Approved</option>
                                                                            <option value="to-deliver" class="bg-warning text-danger" <?= $row['orderStatus'] == 'to-deliver' ? 'selected' : '' ?>>To be Deliver</option>
                                                                            <option value="reject" class="bg-danger text-light" <?= $row['orderStatus'] == 'reject' ? 'selected' : '' ?>>Reject</option>
                                                                        </select>
                                                                    </div>
