<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editOrderStatus'])) {
        ...
        $additionalSql = "SELECT * FROM user_delivery_tbl WHERE orderDate = '$orderDate' AND orderTime= '$orderTime' AND orderStatus = 'to-deliver'";
        ...
        while ($additionalRow = mysqli_fetch_assoc($additionalResult))
        {
            ...
            // var_dump($rowID); 
            $updateQuery = "UPDATE user_delivery_tbl SET orderStatus = '$newOrderStatus' WHERE ID = '$rowID' AND orderStatus = 'to-deliver'";
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4" style="background-color: #F2E8C6;">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom border-dark">
                    <h1 class="h2" id="currentPage"><i class="fa-solid fa-truck"></i>&nbsp Deliver > To be deliver</h1>
                </div>
                    ...
                    <table class="border border-secondary" id="tableDineIn">
                        ...
                        <tbody class="border border-secondary">
                            <?php
                                ...
                                $sql = "SELECT *
                                FROM user_delivery_tbl WHERE orderStatus = 'to-deliver' AND modeOfClaim = 'delivery'
                                                                ...
                                                                <form method="POST">
                                                                    ...
                                                                    <div class="input-group mt-1">
                                                                        ...
                                                                        <select class="font-num form-control text-center" name="orderStatus" id="orderStatus" type="text" value="<?= $row['orderStatus']; ?>">
                                                                            <option value="to-received" class="bg-warning text-danger" <?= $row['orderStatus'] == 'to-received' ? 'selected' : '' ?> disabled>To be Deliver</option>
                                                                            <option value="complete" class="bg-success text-light" <?= $row['orderStatus'] == 'complete' ? 'selected' : '' ?>>Completed</option> 
